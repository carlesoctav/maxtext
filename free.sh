...
case "${MODEL_NAME}" in
  gemma3-1b)
    ...
    DEFAULT_CHECKPOINT_PATH="${REPO_ROOT}/models/gemma31b/0/items"
    ;;
...
if [[ "${LOAD_PARAMETERS_PATH}" == *"/scanned/"* ]]; then
  echo "warning: free.sh usually expects an unscanned checkpoint, but LOAD_PARAMETERS_PATH looks scanned." >&2
fi

TOKENIZER_PATH="${TOKENIZER_PATH:-${DEFAULT_TOKENIZER}}"
BASE_OUTPUT_DIRECTORY="${BASE_OUTPUT_DIRECTORY:-${REPO_ROOT}/output/${MODEL_NAME}-free}"
RUN_NAME="${RUN_NAME:-${MODEL_NAME}-sft-free-$(date +%Y%m%d-%H%M%S)}"
